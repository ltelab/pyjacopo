...
def polar_to_cartesian(radar, sweep, field_name, cart_res = 75,
    ...
    if pol_data.dtype == int:
        pol_data = pol_data.astype(float)
    
    try:
        is_ppi = radar.sweep_mode['data'][0] == 'ppi'
    except:
        is_ppi = ((radar.sweep_mode['data'].data[0][0].decode()=='p') &
                  (radar.sweep_mode['data'].data[0][1].decode()=='p') &
                  (radar.sweep_mode['data'].data[0][2].decode()=='i'))
    if mapping:
