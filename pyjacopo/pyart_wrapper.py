...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  2 15:56:18 2018
...
import pyjacopo.constants as constants




def _create_metadata(config, dname, proc_data, header):
    '''
    def _create_metadata(config)

    PURPOSE:
        Creates a set of MXPOL metadata information to include in the PyART 
        class instances created during the radar processing

    INPUTS:
        config: user configuration stucture
        process_data : dictionary returned by process_dataset

    OUTPUTS:
        metadata: dictionary of metadata for MXPOL
    '''
    
    cfg_dataset = config['products']['datasets'][dname]
    
    metadata = {}
    
    lat = config['location']['radar_position']['latitude']
    metadata['GPSCoordSystem'] = constants.GPS_COORDSYSTEM
    metadata['Hemisphere'] = 'N' if lat > 0 else 'S'
    metadata['Latitude_unit'] = constants.LATITUDE_UNIT
    metadata['Latitude_value'] = lat
    metadata['Longitude_unit'] = constants.LONGITUDE_UNIT
    metadata['Longitude_value'] = config['location']['radar_position']['longitude']
    metadata['Altitude_unit'] = constants.ALTITUDE_UNIT
    metadata['Altitude_value'] = config['location']['radar_position']['altitude']
    metadata['TimeAssignScan_unit'] = constants.TIME_ASSIGN_SCAN_UNIT
    metadata['TimeAssignScan_value'] = proc_data['Time'][-1]
    metadata['RadarParameters'] = constants.RADAR_PARAMETERS
    metadata['Radar_name'] = config['main']['radar_name']
    metadata['Radar_description'] = config['main']['radar_name']


    if 'Va' in proc_data.keys():
        nyq_vel = np.mean(proc_data['Va'])
    elif 'Va1' in proc_data.keys():
        nyq_vel =  np.abs((proc_data['Va1'] * proc_data['Va2']) /
                          (proc_data['Va1'] - proc_data['Va2']))
    metadata['NyquistVelocity_unit'] = constants.NYQUIST_VELOCITY_UNIT
    metadata['NyquistVelocity_value'] = nyq_vel
    metadata['AzOffset_unit'] = constants.AZ_OFFSET_UNIT
    metadata['AzOffset_value'] = config['location']['az_offset']
    metadata['AzOffset_description'] = constants.AZ_OFFSET_DESCRIPTION
    metadata['RadarFreq_unit'] = constants.RADAR_FREQ_UNIT
    metadata['RadarFreq_value'] = config['location']['radar_frequency']
    metadata['BW3dB_unit'] = constants.BW3DB_UNIT
    metadata['RBW3dB_value'] = config['location']['radar_beamwidth']
    metadata['BW3dB_description'] = constants.AZ_OFFSET_DESCRIPTION
    metadata['BW3dB_description'] = constants.AZ_OFFSET_DESCRIPTION
    if header['servmode'] in [0,2]:
        acq_param = 'FilterBW PRI1 PRIT PulseWidth RangeResolution RangeToFirstGate NumRanges NoiseH NoiseV'
    else:
        acq_param = 'FilterBW PRI1 PRI2 PRI3 PRIT PulseWidth RangeResolution RangeToFirstGate NumRanges NoiseH NoiseV'
    metadata['AcquisitionParameters'] = acq_param
    if header['servmode'] in [2,3,4]:
        metadata['FilterBW_unit'] = constants.FILTER_BW_UNIT
        metadata['FilterBW_value'] = header['bandwidth']
    metadata['PRI1_unit'] = constants.PRI_UNIT
    metadata['PRI1_value'] = header['pri1']
    if 'pri2' in header.keys():
        metadata['PRI2_unit'] = constants.PRI_UNIT
        metadata['PRI2_value'] = header['pri2']
    if 'pri3' in header.keys():
        metadata['PRI3_unit'] = constants.PRI_UNIT
        metadata['PRI3_value'] = header['pri3']
    metadata['PRIT_unit'] = constants.PRI_UNIT
    metadata['PRIT_value'] = header['pri1']
    metadata['PulseWidth_unit'] = constants.PULSEWIDTH_UNIT
    tau = 2 / constants.C * header['rres'] * 10E5
    metadata['PulseWidth_value'] = tau
    metadata['RangeResolution_unit'] =  constants.RANGE_RESOLUTION_UNIT
    metadata['RangeResolution_value'] = header['rres']
    metadata['RangeGateSpacing_unit'] = constants.RANGE_GATE_SPACING_UNIT
    metadata['RangeGateSpacing_value'] = header['rgs']
    metadata['RangeToFirstGate_unit'] =  constants.RANGE_TO_FIRST_GATE_UNIT
    metadata['RangeToFirstGate_value'] = proc_data['Range'][0]
    metadata['NumRanges_unit'] =  constants.NUM_RANGES_UNIT
    metadata['NumRanges_value'] = len(proc_data['Range'])
    metadata['NoiseH(V)_explanation'] =  constants.NOISE_HV_EXPLANATION
    metadata['NoiseH_unit'] = constants.NOISE_H_UNIT
    metadata['NoiseH_value'] = np.nanmean(proc_data['Noise_h'])
    metadata['NoiseV_unit'] = constants.NOISE_V_UNIT
    metadata['NoiseV_value'] = np.nanmean(proc_data['Noise_v'])
    if dname == 'RHI':
        metadata['Azimuth_unit'] = constants.AZIMUTH_UNIT
        metadata['Azimuth_value'] = np.mean(proc_data['Azimuth'])
    elif dname == 'PPI':
        metadata['Elevation_unit'] = constants.ELEVATION_UNIT
        metadata['Elevation_value'] = np.mean(proc_data['Elevation'])
    metadata['MissingParam'] = -999.
    metadata['MissingData_explanation'] = constants.MISSING_PARAM_EXPLANATION
    moreaquparams = 'ScanMode FFTLength CAve PostAve ClutterFilter SumPower'
    metadata['MoreAcqParameters'] = moreaquparams
    metadata['ScanMode'] = constants.SERVMODE_NAMES[header['servmode']]
    if header['servmode'] in [2,3,4]:
        metadata['FFT_length_unit'] = constants.FFT_LENGTH_UNIT
        metadata['FFT_length_unit'] = header['fftl']
    else:
        metadata['FFT_length_unit'] = constants.FFT_LENGTH_UNIT
        metadata['FFT_length_unit'] = 0.
    metadata['CAve_unit'] = constants.CAVE_UNIT
    metadata['CAve_value'] = header['cave']
    metadata['PostAve_unit'] = constants.POST_AVE_UNIT
    metadata['PostAve_value'] = header['postave']

    if header['cfon']:
        metadata['ClutterFilter'] = 'ON'
        metadata['ClutterFilterWidth_unit'] = constants.CLUTTER_FILTER_WIDTH_UNIT
        metadata['ClutterFilterWidth_value'] = header['cfw']
    else:
        metadata['ClutterFilter'] = 'OFF'
        metadata['ClutterFilterWidth_unit'] = constants.CLUTTER_FILTER_WIDTH_UNIT
        metadata['ClutterFilterWidth_value'] = 0.

    if header['sumpower'] == 0:
        metadata['SumPower'] = 'OFF'
    else:
        metadata['SumPower'] = 'ON'

    metadata['CalibrationParameters'] = 'Zcal ZdrOffset'
    metadata['ZCal_dB'] = config['location']['radar_calibration_constant']
    metadata['ZdrOffset_dB'] = config['location']['zdr_offset']
    metadata['CensoringParameters'] = 'CensoringMethod SNRthPower SNRthPhase Rhohvth'
    metadata['CensoringMethod'] = 'SNRthPower_SNRthPhase_Rhohvth'
    metadata['SNRthPower_dB'] = cfg_dataset['processing']['snr_pow']
    metadata['SNRthPhase_dB'] = cfg_dataset['processing']['snr_phase']
    metadata['Rhohvth'] = cfg_dataset['processing']['rhohv_th']
    metadata['PhidpParameters'] = 'PhidpEstMethod'
    metadata['PhidpEstParameters'] = cfg_dataset['processing']['phidp_kdp']['phidp_method']
    metadata['KdpParameters'] = 'KdpEstMethod'
    metadata['KdpEstParameters'] = cfg_dataset['processing']['phidp_kdp']['kdp_method']
    metadata['AttCorrParameters'] = 'AttCorrMethod'
    metadata['AttCorrMethod'] = cfg_dataset['processing']['att_corr']['method']
    metadata['History'] = 'Created '+ datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    metadata['Source'] = 'Radar data processed at LTE-EPFL'
    metadata['Institution'] = 'Laboratoire de Teledetection Environnementale - Ecole Polytechnique Federale de Lausanne'
    metadata['ContactInformation'] = 'http://lte.epfl.ch'
...
def read_cfradial(filename, field_names=None, additional_metadata=None,
    ...
    if 'ray_n_gates' in ncvars:
        ...
        # all following variables are fields
        keys = [k for k, v in ncvars.items()
                if v.dimensions in (('time', 'range'),
                                    ('time','range','nfft'))]
    fields = {}
    for key in keys:
    ...
    if 'nfft' in ncobj.dimensions:
        output.fftvel = _ncvar_to_dict(ncvars['sVel'])
...
def write_cfradial(filename, radar, format='NETCDF4', time_reference=None,
    ...
    if 'nfft' in dir(radar):
        dataset.createDimension('nfft', radar.nfft)
    ...
    for field, dic in radar.fields.items():
        ...
        if len(dic['data'].shape) == 1:
            _create_ncvar(dic, dataset, field, ('nfft'))
        elif len(dic['data'].shape) == 2:
            _create_ncvar(dic, dataset, field, ('time', 'range'))
        else: # 3D
            _create_ncvar(dic, dataset, field, ('time', 'range','nfft'))
    ...
    start_dic = {'data': np.array(start_dt.strftime('%Y-%m-%dT%H:%M:%SZ') + 'Z', dtype='S'),
                 'long_name': 'UTC time of first ray in the file',
                 'units': 'unitless'}
    end_dic = {'data': np.array(end_dt.strftime('%Y-%m-%dT%H:%M:%SZ') + 'Z', dtype='S'),
...
def _create_ncvar(dic, dataset, name, dimensions):
    ...
    kwargs = {'zlib': True, 'complevel':9}  # default is to use compression
...
class PyartMXPOL(pyart.core.Radar):
    '''
    PyartMXPOL(dataset_name, data_dic, config)
    ...
    INPUTS:
        dataset_name: PyART dataset name, 'RHI' or 'PPI'
        data_dic: lvl 0 processed data dictionary, as given by process_data
        config: user configuration stucture

    OUTPUTS:
        A PyART class instance
    '''
    
    def __init__(self, dataset_name, data_dic, config, header,verbose=False):
        
        has_spectral = False # will be checked later on
        
        # Specify loooots of inputs for the Pyart class        
 
        N_sweeps = 1  # Should be adapted if we start to create volume scans
        ...
        for i,k in enumerate(constants.VARNAMES):
            fields[k] = {}
            fields[k]['data'] = []
            fields[k]['long_name'] = constants.LABELS[i]
            fields[k]['units'] = constants.UNITS[i]
            fields[k]['valid_min'] = constants.VMIN[i]
            fields[k]['valid_max'] = constants.VMAX[i]
        ...
        if dataset_name == 'RHI':
            fixed_angle['data'] = [data_dic['Azimuth'][0]]
        elif dataset_name == 'PPI':
            fixed_angle['data'] = [data_dic['Elevation'][0]]

        [N_ranges, N_az] = data_dic['Zh'].shape
        ...
        for j,v in enumerate(constants.VARNAMES):
            if v in data_dic.keys():
                d = data_dic[v] # pyart assumes dimensions to be time, range
                mask = ~data_dic['Mask_data']
                if len(mask.shape) < len(d.shape):
                    # For spectral data, since mask is 2D only
                    mask = np.repeat(mask[:,:,None], d.shape[2], axis=2)
                    has_spectral = True
                    fftl = d.shape[2]
                elif len(mask.shape) > len(d.shape):
                    mask = np.zeros((len(d)))
                    
                fields[v]['data'] = np.ma.array(d, mask = mask, 
                      fill_value = np.nan)
            else:
                if verbose:
                    print('Variable '+v+' was not found in file!')
    
        latitude = {'data' : np.array([config['location']['radar_position']['latitude']] * N_sweeps)}
        longitude = {'data' : np.array([config['location']['radar_position']['longitude']] * N_sweeps)}
        altitude = {'data' : np.array([config['location']['radar_position']['altitude']] * N_sweeps)}
        sweep_number = {'data' : np.arange(0, N_sweeps)}
        sweep_mode = {'data' : np.array([dataset_name.lower()]*N_sweeps)}
        instrument_parameters = {'nyquist_velocity': {'data': [data_dic['Va']]}}
    
        azimuth = {'data' : data_dic['Azimuth']}
        rrange = {'data': data_dic['Range']}
        elevation = {'data': data_dic['Elevation']}
    
        time_units='seconds since 1970-01-01T00:00:00Z'
        time = {'data' : data_dic['Time'],'units': time_units}
    
        # Finally add ranges as an additional variable, for convenience in order to
        # filter gates based on their range
    
        metadata = _create_metadata(config, dataset_name, data_dic, header)
       
        # Add fft length if some variables are 3D
        if has_spectral:
            self.nfft = fftl
        
        # Create PyART instance
        super(PyartMXPOL,self).__init__(time,
             rrange,
             fields,
             metadata, 
             dataset_name.lower(),
